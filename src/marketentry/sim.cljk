...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (MDV, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up mdv-registration-missing) ==")
    ...
    (println "== filing/submit eng-4 (mdv-registration-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up mira-registration-unverified) ==")
    ...
    (println "== filing/submit eng-5 (mira-registration-unverified -> HARD hold) ==")
