(ns marketentry.store
  "SSoT for the MDV market-entry compliance actor, behind a `Store`
  ...
  This engagement schema has TWO boolean-pair fields -- Business
  Registration Act registration
  (`:requires-mdv-registration?`/`:has-mdv-registration?`) and MIRA tax
  registration
  (`:requires-mira-registration?`/`:mira-registration-verified?`) --
  mirroring the AGO reference implementation's TWO boolean pairs
  (`:requires-ao-entity?`/`:has-ao-entity?` plus
  `:requires-nif?`/`:nif-verified?`), because the Maldives dossier
  (`marketentry.facts`) grounds both as independently-checkable
  governor conditions."
...
(defn demo-data
  ...
  lifecycles (draft, submit) plus the governor's own checks:
    ...
    eng-2 -- unknown jurisdiction (\"ATL\", fictitious -- no spec-basis)
    eng-3 -- engagement-fee-mismatch (claimed-fee != base + months x rate)
    eng-4 -- mdv-registration-missing (requires but lacks Business
             Registration Act registration)
    eng-5 -- mira-registration-unverified (requires but lacks verified
             MIRA tax registration)
    eng-6 -- clean, different fee shape (a second honest commit path)"
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kaani Trading Pvt Ltd" :portal "Public Finance Regulation filing"
             :base-fee 400000 :monthly-rate 20000 :monitoring-months 12
             :claimed-fee 640000.0
             :requires-mdv-registration? true :has-mdv-registration? true
             :requires-mira-registration? true :mira-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MDV" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "Public Finance Regulation filing"
             :base-fee 400000 :monthly-rate 20000 :monitoring-months 12
             :claimed-fee 640000.0
             :requires-mdv-registration? true :has-mdv-registration? true
             :requires-mira-registration? true :mira-registration-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Hulhumale Ventures" :portal "Public Finance Regulation filing"
             :base-fee 400000 :monthly-rate 20000 :monitoring-months 12
             :claimed-fee 799000.0
             :requires-mdv-registration? true :has-mdv-registration? true
             :requires-mira-registration? true :mira-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MDV" :status :intake}
    "eng-4" {:id "eng-4" :operator "Villimale Exports" :portal "Public Finance Regulation filing"
             :base-fee 400000 :monthly-rate 20000 :monitoring-months 12
             :claimed-fee 640000.0
             :requires-mdv-registration? true :has-mdv-registration? false
             :requires-mira-registration? true :mira-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MDV" :status :intake}
    "eng-5" {:id "eng-5" :operator "Addu Logistics" :portal "Public Finance Regulation filing"
             :base-fee 400000 :monthly-rate 20000 :monitoring-months 12
             :claimed-fee 640000.0
             :requires-mdv-registration? true :has-mdv-registration? true
             :requires-mira-registration? true :mira-registration-verified? false
             :drafted? false :submitted? false
             :jurisdiction "MDV" :status :intake}
    "eng-6" {:id "eng-6" :operator "Maafushi Civic Systems" :portal "Public Finance Regulation filing"
             :base-fee 300000 :monthly-rate 15000 :monitoring-months 6
             :claimed-fee 390000.0
             :requires-mdv-registration? true :has-mdv-registration? true
             :requires-mira-registration? true :mira-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MDV" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-mdv-registration? has-mdv-registration?
                               requires-mira-registration? mira-registration-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-mdv-registration?)         (assoc :engagement/requires-mdv-registration? requires-mdv-registration?)
    (some? has-mdv-registration?)              (assoc :engagement/has-mdv-registration? has-mdv-registration?)
    (some? requires-mira-registration?)        (assoc :engagement/requires-mira-registration? requires-mira-registration?)
    (some? mira-registration-verified?)        (assoc :engagement/mira-registration-verified? mira-registration-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-mdv-registration? :engagement/has-mdv-registration?
   :engagement/requires-mira-registration? :engagement/mira-registration-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-mdv-registration? (boolean (:engagement/requires-mdv-registration? m))
     :has-mdv-registration? (boolean (:engagement/has-mdv-registration? m))
     :requires-mira-registration? (boolean (:engagement/requires-mira-registration? m))
     :mira-registration-verified? (boolean (:engagement/mira-registration-verified? m))
