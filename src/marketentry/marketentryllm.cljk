(ns marketentry.marketentryllm
  ...
  the MDV public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-mdv-registration?=" (:has-mdv-registration? e)
                        " mira-registration-verified?=" (:mira-registration-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-mdv-registration? e))
                              (:has-mdv-registration? e))
                          (or (not (:requires-mira-registration? e))
                              (:mira-registration-verified? e)))
                   0.9 0.3)}))
