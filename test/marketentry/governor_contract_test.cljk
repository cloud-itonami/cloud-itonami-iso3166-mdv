  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kaani Trading Pvt Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kaani Trading Pvt Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest mdv-registration-missing-is-held-and-unoverridable
  (testing "missing Business Registration Act registration -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:mdv-registration-missing} (-> (store/ledger db) last :basis)))
...
(deftest mira-registration-unverified-is-held-and-unoverridable
  (testing "unverified MIRA tax registration when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:mira-registration-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
